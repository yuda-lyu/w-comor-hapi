...
(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory(require('@hapi/hapi')) :
  typeof define === 'function' && define.amd ? define(['@hapi/hapi'], factory) :
  (global = global || self, global['ht-server'] = factory(global['@hapi/hapi']));
}(this, function (Hapi) { 'use strict';

  Hapi = Hapi && Hapi.hasOwnProperty('default') ? Hapi['default'] : Hapi;
  ...
  function arrhas(vtar, vhas) {
    ...
    if (isestr(vhas)) {
      vhas = [vhas];
    } else if (isearr(vhas)) ; else {
      return false;
    } //由vtar各元素當中，若存在vhas內任一元素則回傳true，反之回傳false


    for (var i = 0; i < vtar.length; i++) {
      for (var j = 0; j < vhas.length; j++) {
        if (vtar[i] === vhas[j]) {
          return true;
        }
      }
  ...
  function HtServer(opt) {
    ...
    function authenticate(token) {
      ...
      if (isfun(opt.authenticate)) {
        opt.authenticate(token).then(function (vd) {
          pm.resolve(vd);
        });
      } else {
        pm.resolve(true);
    ...
    function _execFunction() {
      _execFunction = asyncToGenerator(
      /*#__PURE__*/
      regenerator.mark(function _callee(data) {
        var token, vd, func, input, res, output;
        return regenerator.wrap(function _callee$(_context) {
          while (1) {
            switch (_context.prev = _context.next) {
              case 0:
                console.log("Server[port:".concat(opt.port, "][api:").concat(opt.apiName, "]: "), data); //token

                token = getdtvstr(data, 'token'); //vd
              ...
              case 4:
                ...
                //func
                func = getdtvstr(data, 'func'); //input

                input = getdtv(data, 'input'); //getFuncs
                ...
                _context.next = 12;
                return opt.filterFuncs(token, funcs);
              ...
              case 26:
                return _context.abrupt("return", JSON.stringify({
                  err: "can not authenticate token: ".concat(token)
                }));
    ...
    function newServer() {
      return _newServer.apply(this, arguments);
    } //serverHapi


    function _newServer() {
      _newServer = asyncToGenerator(
      /*#__PURE__*/
